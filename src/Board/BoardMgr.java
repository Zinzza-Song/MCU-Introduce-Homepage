...
public class BoardMgr {
	...
	public void insertBoard(HttpServletRequest req) {
		...
		try {
			...
			sql = "insert board(mem_name,content,title,ref,pos,depth,reg_date,pass,count,likes,filename,filesize)";
	...
	try {
		con = pool.getConnection();
	           sql = "insert board (mem_name,content,title,ref,pos,depth,reg_date,pass,count,likes)";
		sql += "values(?,?,?,?,?,?,now(),?,0,0)";
	...
	public int erase(String title){
		...
		try {
			...
			pstmt = conn.prepareStatement("select title from board where title=?");
			...
			if (rs.next()) {
					...
					pstmt = conn.prepareStatement("delete from board where title=?");
					pstmt.setString(1, title);
					pstmt.executeUpdate();
					x = 1;
